...
public class HtmlEmailSenderImpl implements HtmlEmailSender {
	...
	public void send(Email email) throws Exception {
		...
		final String from = "service@bettbio.com"; // any email should be sent
													// from this account
		    ...
		    MimeMessagePreparator preparator = new MimeMessagePreparator() {
			  	public void prepare(MimeMessage mimeMessage) throws MessagingException, IOException {
			  		...
			  		if (emailConfig != null) {
			  			...
			  			prop.put("mail.smtp.auth", (emailConfig.isSmtpAuth() == false ? "true" : "false"));
			  			prop.put("mail.smtp.starttls.enable", (emailConfig.isStarttls() == false ? "true" : "false"));
			  		...
			  		Template textTemplate = freemarkerMailConfiguration
			  				.getTemplate(new StringBuilder(TEMPLATE_PATH).append("").append("/").append(tmpl).toString());
			  		textTemplate.setEncoding(CHARSET);
...
//			  				.getTemplate(new StringBuilder(TEMPLATE_PATH).append("").append("/").append(tmpl).toString());
//			  		htmlTemplate.setEncoding(CHARSET);
...
//			  		mp.addBodyPart(htmlPart);
	...
	/**
	 * below are code which works in old version, but will send 2 copies of HTML
	 * email.
	 * 
	 * <pre>
	 * MimeMessagePreparator preparator = new MimeMessagePreparator() {
