...
public class OrderServiceImpl  extends SalesManagerEntityServiceImpl<Long, Order> implements OrderService {
    ...
    private OrderTotalSummary caculateShoppingCart( final ShoppingCart shoppingCart, final Customer customer, final MerchantStore store, final Language language) throws Exception {
        ...
        /*// refresh the price by customer and brand discount
        Set<com.salesmanager.core.business.shoppingcart.model.ShoppingCartItem> lineItems = shoppingCart.getLineItems();
        if (lineItems != null) {
 	       List<Long> productIds = new ArrayList<Long>();
 	       for(com.salesmanager.core.business.shoppingcart.model.ShoppingCartItem curItem:lineItems){
 	    	   productIds.add(curItem.getProductId());
 	       }
 	       List<Object[]> discountList = brandDiscountService.getDiscountRateByProductIdList(productIds);
 	       for(com.salesmanager.core.business.shoppingcart.model.ShoppingCartItem curItem:lineItems){
 	    	   Double finalDiscountRate = ProductPriceUtils.calcFinalListDiscount(customer, curItem.getProductId(), discountList);
 	    	   if (finalDiscountRate == null || finalDiscountRate < 0){
 	    		   continue;
 	    	   }
 	    	   double discountPrice = curItem.getProductPrice().getProductPriceAmount().doubleValue() * finalDiscountRate;
 	    	   curItem.setItemPrice(new BigDecimal(discountPrice));
 	    	   curItem.setSubTotal(new BigDecimal(discountPrice * curItem.getQuantity()));
 	       }
        }*/
     ...
     */
    @Override
    public OrderTotalSummary calculateShoppingCartTotal(final Customer customer,
