...
public class OrderDaoImpl  extends SalesManagerEntityDaoImpl<Long, Order> implements OrderDao {
	 ...
	 */
	@Override	 
	public double getTotalMoney(OrderCriteria criteria){
		//add select
		StringBuilder countBuilderSelect = new StringBuilder();
	...
	public OrderList listByOrderCriteria(OrderCriteria criteria) {
		if (criteria.getStatus() != null && criteria.getSecondStatus() == null){
			criteria.setSecondStatus(criteria.getStatus());
		}
		Date dt = new Date();
		...
		query.from(qOrder)
			...
			.orderBy(qSubOrder.id.desc());
			// 2017-4-6: change to search by suborder id. Below are old code
		...
		if(!StringUtils.isEmpty(criteria.getFindName())) {
			...
			pBuilder.andAnyOf(qOrder.billingUserName.like(new StringBuilder().append("%").append(criteria.getFindName()).append("%").toString()),
					...
					qSubOrder.id.like(new StringBuilder().append("%").append(criteria.getFindName()).append("%").toString()));
			// 2017-4-6: change to search by suborder id. Below are old code
			//qOrder.id.like(new StringBuilder().append("%").append(criteria.getFindName()).append("%").toString()));
		...
		if(criteria.getStatus() != null){
			if(criteria.getStatus().name().equalsIgnoreCase("REVIEW")){
				...
				BooleanExpression st1 = qOrder.status.eq(criteria.getStatus())
						.and(qOrder.processType.eq(GlobalConstants.OrderProcessType_PayFirst));
				BooleanExpression st2 = qOrder.status.eq(criteria.getSecondStatus()).and(qOrder.processType
						.eq(GlobalConstants.OrderProcessType_ShipFirst).or(qOrder.processType.isNull()));
				pBuilder.andAnyOf(st1, st2);
			}			
		}
		...
		orderList.setTotalCount((int)(query.distinct().count()));
		return orderList;
	...
	public OrderList listStoreByOrderCriteria(OrderCriteria criteria) {
		...
		OrderList orderList = new OrderList();
/*		StringBuilder countBuilderSelect = new StringBuilder();
		countBuilderSelect.append("select count(distinct o) from Order as o");
		...
		if (!StringUtils.isEmpty(criteria.getFindName())) {
			countBuilderSelect.append(" ,OrderProduct as p, Customer as cst ");
			countBuilderWhere.append(" and o.id = p.subOrder.order.id and o.customerId = cst.id ");
			countBuilderWhere.append(" and (p.productName like:pn or o.billingUserName like:cn or o.id like '%"
					+ criteria.getFindName() + "%' or cst.compnay like '%" + criteria.getFindName() + "%')");
		}
		...
		if(criteria.getStatus() != null){
			countBuilderWhere.append(" and ((o.status=:st and o.processType='pay_first') or (o.status=:st2 and (o.processType='ship_first' or o.processType is null)))");
		}
		...
		if(criteria.getStatus() != null){
			...
			countQ.setParameter("st2", criteria.getSecondStatus());
		}
		if(!StringUtils.isBlank(criteria.getBeginDatePurchased())){
			//最近3个月的数据
			if(criteria.getBeginDatePurchased().equalsIgnoreCase("3")){
		...
		query.from(qOrder, qCustomer)
			...
			//.leftJoin(qOrderProduct.downloads).fetch()
			//.leftJoin(qOrderProduct.orderAttributes,qOrderProductAttribute).fetch()
			//.leftJoin(qCustomer).on(qOrder.customerId.eq(qCustomer.id))
			//.leftJoin(qOrderProduct.prices).fetch()
			//.where(qSubOrder.merchant.id.eq(criteria.getStoreid()))
			.orderBy(qOrder.id.desc());
			BooleanBuilder pBuilder = new BooleanBuilder();
			pBuilder.and(qOrder.customerId.eq(qCustomer.id));
			if(criteria.getStoreId() != -1l){
				pBuilder.and(qSubOrder.merchant.id.eq(criteria.getStoreId()));
			}
		
		if (!StringUtils.isEmpty(criteria.getFindName())) {
			// pBuilder.and(qOrderProduct.productName.like(new
			// StringBuilder().append("%").append(criteria.getFindName()).append("%").toString()));
			String searchValue = new StringBuilder().append("%").append(criteria.getFindName()).append("%").toString();
			pBuilder.andAnyOf(qOrder.billingUserName.like(searchValue), 
					qOrderProduct.productName.like(searchValue),
					qSubOrder.id.like(searchValue),
					qOrder.id.like(searchValue), 
					qCustomer.compnay.like(searchValue));

		}
		if(criteria.getStatus() != null){
			BooleanExpression st1 = qOrder.status.eq(criteria.getStatus())
			.and(qOrder.processType.eq(GlobalConstants.OrderProcessType_PayFirst));
			BooleanExpression st2 = qOrder.status.eq(criteria.getSecondStatus())
					.and(qOrder.processType.eq(GlobalConstants.OrderProcessType_ShipFirst).or(qOrder.processType.isNull()));
			pBuilder.andAnyOf(st1, st2);
		}
		if(!StringUtils.isBlank(criteria.getBeginDatePurchased())){
			//最近3个月的数据
		...
		//if(pBuilder!=null) {
			query.where(pBuilder);
		//}
		long count = query.distinct().count();
		orderList.setTotalCount((int) count);
	...
	public double getTotalMoney(Customer customer) {
		...
		//add select
				...
				// 默认是先发货
				String sqlShipFirst = "(o.processType!='" + GlobalConstants.OrderProcessType_ShipFirst 
						+"' and s.status in ('ORDERED', 'SHIPPED', 'RECEIPT', 'PAID'))";
				String sqlPayFirst = "(o.processType='" + GlobalConstants.OrderProcessType_ShipFirst 
						+"' and s.status in ('ORDERED', 'PAID'))";
				countBuilderWhere.append(" and (");
				countBuilderWhere.append(sqlShipFirst);
				countBuilderWhere.append(" or ");
				countBuilderWhere.append(sqlPayFirst);
	...
	public int getOrderByCount(Long customerId) {
		...
		countBuilderSelect.append("select count(o) from Order as o where o.customerId="+customerId+"");
