...
import com.salesmanager.core.business.common.model.QBasedataType;
import com.salesmanager.core.business.generic.dao.SalesManagerEntityDaoImpl;

@Repository("basedataTypeDao")
public class BasedataTypeDaoImpl extends SalesManagerEntityDaoImpl<Long, BasedataType>
		implements BasedataTypeDao {

	@Override
	public List<Object[]> getStoreName(String type) {
		// TODO Auto-generated method stub
		StringBuilder qs = new StringBuilder();
		qs.append("select distinct BASEDATA_TYPE_ID as id,NAME as name,IFNULL(ELT(INTERVAL(CONV(HEX(left(CONVERT(NAME USING gbk),1)),16,10), ");
		...
		qs.append(" 'Q','R','S','T','W','X','Y','Z'),LEFT(NAME,1)) as pinyin  from BASEDATA_TYPE  where type='");
		qs.append(type);
	...
	public List<BasedataType> getByName(String name) {
		...
		QBasedataType qBasedataType = QBasedataType.basedataType;
		JPQLQuery query = new JPAQuery (getEntityManager());
		query.from(qBasedataType)		
		.orderBy(qBasedataType.id.asc());
		
		BooleanBuilder pBuilder = new BooleanBuilder();
		
		pBuilder.and(qBasedataType.name.equalsIgnoreCase(name));
		
		if(pBuilder!=null) {
			query.where(pBuilder);
		}
		
		return query.list(qBasedataType);		
	}
}
