...
public class UserDaoImpl extends SalesManagerEntityDaoImpl<Long, User>
	...
	@Override
	public User getByUserMobile(String mobile) {
	...
	public int queryByAccount(int index,String account) {
		...
		case 1:
			count = (Long) getEntityManager().createQuery(
					"select count(u) from User as u where u.mobile='"
	...
	public User queryUserByEmail(String meail) {
		...
		query.from(qUser).innerJoin(qUser.groups, qGroup).fetch()
				.innerJoin(qUser.merchantStore).fetch()
				.leftJoin(qUser.defaultLanguage).fetch()
				.where(qUser.adminEmail.eq(meail));
	...
	@Override
	public Map<String, User> getPossibleUsersByLogonName(String logonName) {
		Map<String, User> result = new HashMap<String, User>();
		// try user name
		User user = getByUserName(logonName);
		if (user != null){
			result.put(Constants.LOGON_BY_NAME, user);
		}
		// try mobile number
		user = getByUserMobile(logonName);
		if (user != null){
			result.put(Constants.LOGON_BY_MOBILE, user);
		}
		
		// try email
		user = queryUserByEmail(logonName);
		if (user != null){
			result.put(Constants.LOGON_BY_EMAIL, user);
		}
		return result;
	}
