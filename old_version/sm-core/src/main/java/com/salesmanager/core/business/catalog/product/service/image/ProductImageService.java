...
public interface ProductImageService extends SalesManagerEntityService<Long, ProductImage> {
	 ...
	 * @throws ServiceException
	 */
	void addProductRelatedImage(Product product, ImageContentFile inputImage)
			throws ServiceException;
	
	OutputContentFile getProductRelatedImage(String storeCode, String productCode,
			String fileName, final String related) throws ServiceException;

	/**
	 * remove  a Product Certificate or Proof Related Image to the persistence and an entry to the CMS
	 * @param product
	 * @param certificateImage 图片名
	 * @param productCertificate 类型（certificate or proof)
	 */
	void removeProductRelatedImage(Product product, String certificateImage, FileContentType related) throws ServiceException;
