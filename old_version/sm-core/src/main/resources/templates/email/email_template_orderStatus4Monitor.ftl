...
<div class="content">
	...
	订单号: ${orderId?c}<br/>
	当前状态: ${orderStatus}<br/>
	日期: ${orderDate?datetime?string('yyyy-MM-dd HH:mm:ss')}<br/> 
	</p>
	<!--<hr/>-->
	<br/>
	<!-- order dertail -->
	<div class="content-table">
		...
		<table>
			<tr>
				<td colspan="4">卖家：${subOrder.storeName!}</td>
				<td>
		...
		<#list subOrder.orderProducts as product>
			...
			<tr><td  colspan="6">
			<#if subOrder.deliveryNumber?has_content>
			物流信息: <strong>${subOrder.deliveryCode} ${subOrder.deliveryNumber}
			</#if>
			</td></tr>
		</table>
	</div>
