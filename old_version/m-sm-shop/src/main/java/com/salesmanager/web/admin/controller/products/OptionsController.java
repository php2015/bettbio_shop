...
import com.salesmanager.web.admin.controller.products.facade.OptionFacade;
...
public class OptionsController {
	...
	@Autowired 
	OptionFacade opFacade;
	
	private static final Logger LOGGER = LoggerFactory.getLogger(OptionsController.class);
	
	
	@PreAuthorize("hasRole('PRODUCTS')")
	@RequestMapping(value="/admin/options/list.html", method={RequestMethod.GET,RequestMethod.POST})
	public String displayOptions(@Valid @ModelAttribute("criteria") Criteria criteria,Model model, HttpServletRequest request, HttpServletResponse response,@RequestParam(value = "page", defaultValue = "1") final int page) throws Exception {
		...
		ReadableOptionList olist = opFacade.getByCriteria(criteria, language);
		
		model.addAttribute( "options", olist);
		if(olist!=null) {
		...
		setMenu(model,request,"catalogue-options-list");		
	...
	@PreAuthorize("hasRole('PRODUCTS')")
	@RequestMapping(value="/admin/options/removes.html", method=RequestMethod.POST, produces="application/json")
	public @ResponseBody String deleteOptions(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		try{
			...
			delteAll.setService(productOptionService);
	...
	private String displayOption(Long optionId, HttpServletRequest request, HttpServletResponse response,Model model,Locale locale) throws Exception {
		...
		if(optionId == null){
			this.setMenu(model, request, "catalogue-options-create");
		}else{
			this.setMenu(model, request, "catalogue-options-list");
		}
		MerchantStore store = (MerchantStore)request.getAttribute(Constants.ADMIN_STORE);
