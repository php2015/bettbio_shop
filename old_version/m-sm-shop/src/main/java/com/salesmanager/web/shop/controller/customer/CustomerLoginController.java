	...
	public @ResponseBody String logon(@ModelAttribute SecuredCustomer securedCustomer, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
        	...
        	if ("password".equals(loginType)){
        		customerModel = customerFacade.getCustomerForLogin(securedCustomer.getUserName());
        	}else {
        		mobileNumber = request.getParameter("mobile");
        		customerModel = customerFacade.getCustomerForLogin(mobileNumber);
        	}
            if(customerModel==null) {
            	jsonObject.setStatus(AjaxResponse.RESPONSE_STATUS_NO_CUSTOMER);
            ...
            if ("password".equals(loginType)){
            	...
            	customerFacade.authenticate(customerModel, customerModel.getId().toString(), request.getParameter("smsCode"));
            }
