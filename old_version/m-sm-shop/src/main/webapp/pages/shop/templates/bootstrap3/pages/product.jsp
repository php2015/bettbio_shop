...
<div class="body">
	...
	<div class="row product-header">
			...
			<div id="img" class="col-xs-12" style="padding:0px;margin:0px;">
				<c:if test="${product.image!=null}">											
					<span id="mainImg"><a href="<c:url value="/shop/dispalyImage.html?ipath=${product.image.imageUrl}"/>" target="blank"><img class="product-img" style="width:100%"  alt="<c:out value="${product.description.name}"/>" src="<c:url value="${product.image.imageUrl}"/>" ></a></span>	
					</c:if>
			</div>
							...
							<td style="line-height:25px!important">
								<c:choose>
									...
									<c:otherwise>
										...
										<c:if test="${empty price.discountType}">
											${price.price }
										</c:if>
										<c:if test="${price.discountType == 'list_price'}">
											${price.price }
										</c:if>
										<c:if test="${price.discountType == 'customer_discount' || price.discountType == 'brand_discount'}">
											${price.discountPrice} <br/>
											<s style="color:gray; font-size:10px;">原价 ${price.price }</s>
										</c:if>
										<c:if test="${price.discountType == 'timed_promotion'}">
											${price.discountPrice} <br/>
											<s style="color:gray; font-size:10px;">原价 ${price.price }</s>
										</c:if>
									</c:otherwise>
	...
	<div>
		...
		<div id="test" class="productdesc featurette productdesc-even-background">
			<div>
				<div class="productdesc-title"><s:message code="label.productedit.producttestdesc" text="Postalcode"/>/<s:message code="label.product.selfproof" text="Postalcode"/></div>
			</div>
