...
<script src="<c:url value='/resources/js/check-functions.js'/>?v=<%=new java.util.Date().getTime()%>"></script>
...
$(function(){
   ...
   $("#captchaResponseField").keyup(function(){
	   var value = $("#captchaResponseField").val();
	   if (value.length < 4){
		   return;
	   }
   ...
   //发送验证码
   $("#messageSecond_notused").click(function(){
...
function showErro(status){
	 if(status==-2){
		 $("#smsCode").html('<s:message code="NotEmpty.contact.captchaResponseField" text="Message" />');
	 }else if(status==-3){
		 $("#smsCode").html('<s:message code="validaion.recaptcha.not.matched" text="Message" />');
		 changeValidateImg();
	 }else if(status==-1){
		 $("#smsCode").html('<s:message code="label.generic.phone.message.fail" text="Message" />');
	 }else {
		 $("#smsCode").html('<s:message code="registration.phone.already.exists" text="Message" />');
	 }
	 $("#smsCode").show();
}
//改变按钮状态
	...
	<div style="background: #eeeeee;height:100%">
		...
		<div style="background:#ffffff;padding-top:24px; height:72px;">
			<div style="padding-left:10px;" id="yd">
				...
				<span class="padding-img" style="font-size:16px;">2.设置密码和邮箱</span>
		...
		<form:form method="post" action="${register_url}" id="registrationForm" class="form-horizontal" commandName="customer">
		    ...
		    <div style="background:#ffffff;padding-top:20px;padding-bottom:20px;font-size:16px;position: relative;">
				<span class="padding-register" style="font-size:16px;">请输入短信码：</span><input type="text" style="width:100px;border: 0px;height:30px;" name="checkPhonecode"  id="checkPhonecode" success="false" class="sminput"></input>
				 
				 <button style="padding-right:5px;background: #ffffff;" id="btnSendCode" type="button" class="btn pull-right" disabled value="<s:message code="label.generic.phone.message" text="Phone"/>" onclick="sendPhoneNumtwo()" >
