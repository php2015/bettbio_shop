								...
								if (p.prices[j].special != null && p.prices[j].special !=''){
									...
									if (p.prices[j].priceValue != null && p.prices[j].priceValue == '0') {
										...
										if (specPrice.discountType == null){
											displayPrice = '询价' +"<br/><a style='color:blue;' href='" + loginPageUrl +"'>登录可见折扣</a>";
										}else if (specPrice.discountType == "list_price"){
											// nothing to do
										}else if (specPrice.discountType == "timed_promotion"){
											displayPrice = "<span style='border: 1px solid blue;'>" +'限时促销' + '<span>';
										}else{
											displayPrice = '<span style="border: 1px solid blue;">'+getDiscountDisplay(specPrice.discountRate) + '折</span>询价';
										}
										
										/*
										if (p.prices[j].discountRate == null){
											// 没有折扣，那就是已登录，但是没有设置折扣。 什么也不用改
										}else{
											if (p.prices[j].discountRate < 0){
												...
												// 已登录，有折扣
												displayPrice = "<span style='border: 1px solid blue;'>" +getDiscountDisplay(p.prices[j].discountRate) + "折</span> " +'询价';
											}
										}*/
										priceHtml += '<div class="col-xs-6" style="padding-left:0px!important;color:blue">' + displayPrice;
										priceHtml += '</div>';
										//priceHtml += '<div class="col-xs-3">';
										//priceHtml += '</div>';
									} else {
										// 计算应该显示的价格
										var displayPrice = specPrice.priceValue;
										if (specPrice.discountType == null){
											displayPrice = specPrice.priceValue +"<br/><a style='color:#FF6C00;' href='" + loginPageUrl +"'>登录可见折扣</a>";
										}else if (specPrice.discountType == "list_price"){
											// nothing to do
										}else if (specPrice.discountType == "timed_promotion"){
											displayPrice = "<span style='border: 1px solid #FF6C00;'>" +'限时促销' + "</span> " +specPrice.discountPrice;
											displayPrice = displayPrice + "<br/><s style='color:gray;font-size:10px;'>原价 " + specPrice.priceValue + "</s>";
										}else{
											displayPrice = "<span style='border: 1px solid #FF6C00;'>" +getDiscountDisplay(specPrice.discountRate) + "折</span> " +specPrice.discountPrice;
											displayPrice = displayPrice + "<br/><s style='color:gray;font-size:10px;'>原价 " + specPrice.priceValue + "</s>";
										}
										...
										if (p.prices[j].discountRate == null){
											...
											if (p.prices[j].discountRate < 0){
												// 未登录
												displayPrice = p.prices[j].price +"<br/><a style='color:#FF6C00;' href='" + loginPageUrl +"'>登录可见折扣</a>";
											}else{
												// 已登录，有折扣
												displayPrice = "<span style='border: 1px solid #FF6C00;'>" +getDiscountDisplay(p.prices[j].discountRate) + "折</span> " +p.prices[j].discountPrice;
												displayPrice = displayPrice + "<br/><s style='color:gray;font-size:10px;'>原价 " + p.prices[j].price + "</s>";
											}
										}*/
										priceHtml += '<div class="col-xs-6" style="padding-left:0px!important;color:#FF6C00">'+ displayPrice;
										priceHtml += '</div>';
										//priceHtml += '<div class="col-xs-1">&nbsp;';
										//priceHtml += '</div>';
									}
								}
