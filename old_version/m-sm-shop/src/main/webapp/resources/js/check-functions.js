    ...
    $.ajax({  
		type: 'POST',
		  ...
		  success: function(response) {
			  if(response.response.status==0) {
					curMessageCount = 30;
		    		 ...
		    		 //$("#messageSucces").show();
		    		 //$("#messageFail"+obj).hide();
					 var randomCode = response.response._randomNumber;
					 if (randomCode != null){
						 alert("短信验证码发送成功: " + randomCode);
						 $('#checkPhonecode').val(randomCode);
						 $('#smsCode').hide();
						 $("#checkPhonecode").focus();
						 $('#checkPhonecode').attr("success","true");
					 }else{
						 alert("验证码已经发送到您的手机，请查看短信消息。");
					 }
		    	 }else{
		    		 showErro(response.response.status);;
		    		 window.clearInterval(messageInterValObj);
		 ...
		 error: function( textStatus, errorThrown) {
			 ...
			 $("#messageFail").show();
	    	 if (messageInterValObj != null) {
				window.clearInterval(messageInterValObj);
				messageInterValObj = null;
			 }
	         $("#btnSendCode").removeAttr("disabled");
	         //alert(textStatus); 
		 }
...
function SetRemainTime(obj) {
	if(obj==null||obj=='undefined'||obj=='') obj = '';
            if (curMessageCount == 0) {                
                window.clearInterval(messageInterValObj);
                $("#messageSecond"+obj).html('点击发送');
                $("#btnSendCode"+obj).removeAttr("disabled");
            }
            else {
            	curMessageCount--;
                $("#messageSecond"+obj).html(curMessageCount+"秒");
            }
        }