...
public class ProductsController {
	...
	@PreAuthorize("hasRole('DIAMOND_PRODUCT_CONTROLLER')")
	@RequestMapping(value="/admin/products/diamondproducts.html", method=RequestMethod.GET)
	public String displayDiamondProducts(Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		
		
		setMenu(model,request,"diamond-product");
		MerchantStore store = (MerchantStore)request.getAttribute(Constants.ADMIN_STORE);
		model.addAttribute("maxDiamondNumber", this.merchantStoreService.getMaxDiamondProductsNumber(store));
		return "admin-diamond-products";
	...
	public @ResponseBody ProductsList displayProduct(HttpServletRequest request, HttpServletResponse response) {
		...
		Boolean filterByDiamondFlag = null;
		String diamondFlag = request.getParameter("diamond");
		if (diamondFlag != null){
			filterByDiamondFlag = Boolean.valueOf("true".equalsIgnoreCase(diamondFlag));
		}
		ProductCriteria criteria = new ProductCriteria();
		criteria.setDiamond(filterByDiamondFlag);
		PaginationData paginaionData=null;
		String pageSize = request.getParameter("perpage");
		if (pageSize != null){
			paginaionData=createPaginaionData(page,Integer.parseInt(pageSize));
		}else{
			paginaionData=createPaginaionData(page,this.productShowPage);
		}
					...
					if(plist!=null) {
						...
						for(Object[] product : plist) {
							try{
								...
								p.setDiamondProduct(product[12]==null?false:new Boolean(product[12].toString()));
								ps.add(p);
							}catch (Exception e){
	...
	// unmark product as diamond product
	@PreAuthorize("hasRole('DIAMOND_PRODUCT_CONTROLLER')")
	@RequestMapping(value = "/admin/products/unmarkdiamond.html", method = RequestMethod.POST, produces = "application/json")
	public @ResponseBody String unmarkProduct(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		AjaxResponse resp = new AjaxResponse();
		if (request.getParameter("productId") != null) {
			try {
				...
				productService.markDiomandProducts(products, false);
				resp.setStatus(AjaxResponse.RESPONSE_STATUS_SUCCESS);
			} catch (Exception e) {
				resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
			}
	...
	// mark product as diamond product
	@PreAuthorize("hasRole('DIAMOND_PRODUCT_CONTROLLER')")
	@RequestMapping(value="/admin/products/markdiamond.html", method=RequestMethod.POST, produces="application/json")
	public @ResponseBody String markProduct(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		AjaxResponse resp = new AjaxResponse();
		if(request.getParameter("productId") !=null){
			try{
				String []sid = request.getParameter("productId").split(",");
				...
				Language language = (Language)request.getAttribute("LANGUAGE");
				int existedDiamondProductNum = productService.getExistedDiomandProductNumber(language, store);
				int maxDiamondProductNum = merchantStoreService.getMaxDiamondProductsNumber(store);
				if (existedDiamondProductNum + products.size() > maxDiamondProductNum){
					Map<String, String> dataMap = new HashMap<String, String>();
					dataMap.put("max", String.valueOf(maxDiamondProductNum));
					dataMap.put("existed", String.valueOf(existedDiamondProductNum));
					dataMap.put("adding", String.valueOf(products.size()));
					resp.setDataMap(dataMap);
					resp.setStatus(AjaxResponse.RESPONSE_STATUS_VALIDATION_FAILED);
					return resp.toJSONString();
				}
				
				productService.markDiomandProducts(products, true);
				resp.setStatus(AjaxResponse.RESPONSE_STATUS_SUCCESS);
			}catch (Exception e){
				resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
			}		
