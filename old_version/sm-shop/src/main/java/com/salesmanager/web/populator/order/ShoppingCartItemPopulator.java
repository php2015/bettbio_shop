...
public class ShoppingCartItemPopulator extends
	...
	public List<ShoppingCartItem> populateList(Set<OrderProduct> set,Language language ) throws ConversionException{
		if(set != null){
			List<ShoppingCartItem> shoppingCartItems = new ArrayList<ShoppingCartItem>();
			for (OrderProduct item : set){
				Product product = productService.getById(item.getProductID()); 
				try {
					ShoppingCartItem shoppingCartItem = shoppingCartService.populateShoppingCartItem(product);
					shoppingCartItem.setQuantity(item.getProductQuantity());
