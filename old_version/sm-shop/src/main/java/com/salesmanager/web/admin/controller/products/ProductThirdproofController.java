...
public class ProductThirdproofController {
	...
	public @ResponseBody List<PinYin> stores(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		try{
			...
			Product dbProduct = productService.getById(Long.parseLong(pid));

			Category category = cmap.get(Long.parseLong(dbProduct.getCategoryId()));
			String proofType="BTYPE_THIRDPROOF"; 
			...
			List<Object[]> stores = basedataTypeService.getStoreName(proofType);
	...
	public String saveProductThirdproof(@Valid @ModelAttribute("thirdproof") ProductThirdproof thirdproof, @RequestParam(required=false, value="image") MultipartFile imagefile, 
		...
		if(code !=null && !StringUtils.isBlank(code)){
			...
			if(bs.getId() !=null){
				...
				//if(!bs.getName().equalsIgnoreCase(code)){
					List<BasedataType> bss = basedataTypeService.getByName(code.toLowerCase().trim());
					if(bss==null || bss.size()==0){
						isCreate=true;
					}else{
						isCreate =true;
						for(int i=0;i<bss.size();i++){
							if(bss.get(i).getType().equalsIgnoreCase(proofType)){
								isCreate=false;
								bs=bss.get(i);
								break;
							}
		...
		//新增
			if(isCreate==true){
				...
				bs.setType(proofType);
				String index ="1";
				try{
					index=PropertiesUtils.getPropertiesValue(proofType+"_INDEX");
				}catch (Exception e){
		...
		if(thirdproof.getThirdproofImage()==null &&  (imagefile==null || imagefile.isEmpty()) && thirdproof.getId()==null){
