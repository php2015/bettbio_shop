...
public class StoreFilter extends HandlerInterceptorAdapter {
	...
	private static final String IS_FIRST_ACCESS = "IS_FIRST_ACCESS";
    ...
    public StoreFilter() {
	   ...
	   public boolean preHandle(
			...
			Boolean isFirstAccess = (Boolean) request.getSession().getAttribute(IS_FIRST_ACCESS);
			// TODO debug
			System.out.println("Access URL = " + request.getRequestURL());
			System.out.println("Now session ID = " + request.getSession().getId());
			System.out.println("    session hashcode = " + request.getSession().hashCode());
			System.out.println("    session object = " + request.getSession());
			if (isFirstAccess == null){
				request.getSession().setAttribute(IS_FIRST_ACCESS, Boolean.TRUE);
			}else{
				request.getSession().setAttribute(IS_FIRST_ACCESS, Boolean.FALSE);
			}
			/**
			...
			try {
				...
				request.getSession().setAttribute(Constants.DEFAULT_LANGUAGE, langs.get(Constants.DEFAULT_LANGUAGE));
				/** 传递store参数，修改访问的商铺 注释掉，不再查看其他商铺**/
				...
				// change 1: clariones, if admin user existed, maybe this just switch
				// from buyer page, so
				// switch the login user to admin user
				String logonAsWho = (String) request.getSession().getAttribute(Constants.LOGON_AS);
				if (customer != null) {
					Authentication auth = SecurityContextHolder.getContext().getAuthentication();
					if (!Constants.LOGON_AS_BUYER.equals(logonAsWho) && auth != null) {
						...
						org.springframework.security.core.userdetails.User secUser = new org.springframework.security.core.userdetails.User(
								String.valueOf(customer.getId()), customer.getPassword(), true, true, true, true, auths);
						UsernamePasswordAuthenticationToken buyerAuth = new UsernamePasswordAuthenticationToken(secUser,
								null, auths);
						buyerAuth.setDetails(auth.getDetails());
	...
	private void getTopCategories(MerchantStore store, Language language, HttpServletRequest request) throws Exception {
			...
			if(store.isUseCache()) {
				...
				//get from the cache
				objects = (List<Category>) cache.getFromCache(categoriesKey.toString());
				...
				if(objects==null) {
					...
					//if(missedContent==null) {
						...
						//objects =categoryService.listTwoDepthByLanguage(store,language);
					
					objects = (List<Category>)request.getSession().getServletContext().getAttribute("topCateGory");
				...
				objects = (List<Category>)request.getSession().getServletContext().getAttribute("topCateGory");
				//objects = categoryService.listByParent(null, language);;
			}
			
			if(objects!=null && objects.size()>0) {
				request.setAttribute("topCateGory",objects);
				...
				//if(categories!=null) {					
					...
					//Map<String,Category> topCategories = categoryService.listTwoDepthByLanguage(store,language);
...
/**	   
 private Map<String, List<Category>> getCategories(MerchantStore store, Language language) throws Exception {
		   ...
		   Map<String, List<Category>> objects = new ConcurrentHashMap<String, List<Category>>();
			...
			Map<String,Category> categories = categoryService.listTwoDepthByLanguage(store,language);
			
			if(categories!=null && categories.size()>0) {				
				//create a Map<String,List<Content>
				 
				for(Object categoryKey : categories.keySet()){ 
