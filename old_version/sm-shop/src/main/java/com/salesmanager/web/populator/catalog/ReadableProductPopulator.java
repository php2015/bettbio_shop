...
public class ReadableProductPopulator extends
	...
	public List<ReadableCategory> getCagoryList(Object object,Category category){
		...
		ReadableCategory rootCtry = new ReadableCategory();
		rootCtry.setProductCount(0);
		rootCtry.setId(category.getId());
		reMap.put(rootCtry.getId(), rootCtry);
		 if (object instanceof InternalLongTermsFacet) {
			 InternalLongTermsFacet ff = (InternalLongTermsFacet) object;
				...
				//子集的子集
   	        		...
   	        		if (cat == null){
   	        		// TODO: debug
   	   	        	 System.out.println("null category with ID " + o.getTerm());
   	        			continue;
   	        		}
...
//						}
//					}
					if (metaEntries.get("brandName") !=null){
						listProduct.setManufacturer(metaEntries.get("brandName").toString());
					}
					//授权书
					...
					if (metaEntries.get("isDiamond") !=null){
						listProduct.setDiamond((boolean) metaEntries.get("isDiamond"));
					}
...
//					this.getSortByQulity(listProducts,listProduct,entry.getSocre());
		...
		for(ProductAvailability price:prices){
			try{
				...
				for(ProductPrice pr:prs){
					...
					if (tmp != null){
						price1.setListPrice(tmp.doubleValue());
					}else{
	...
	public ReadableProduct populate(Product source,
		...
		try {
			...
			target.setDiamond(source.isProductIsDiamond());
			//改成产品编号
