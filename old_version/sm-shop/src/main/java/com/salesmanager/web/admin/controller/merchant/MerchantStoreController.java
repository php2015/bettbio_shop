...
import com.salesmanager.core.business.merchant.service.MerchantRankProfileService;
...
public class MerchantStoreController {
	...
	@Autowired
	MerchantRankService merchantRankService;
	
	@Autowired
	MerchantRankProfileService merchantRankProfileService;
	...
	private String displayMerchantStore(MerchantStore store, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		// get ranks
		List<MerchantRank> ranks = merchantRankService.listRanks();
	...
	public String saveMerchantStore(@Valid @ModelAttribute("store") MerchantStore store, BindingResult result, 
		...
		// handle profile
		MerchantRankProfile  rankProfile = store.getRankProfil();
		if (rankProfile != null){
			boolean isNewProfil = false;
			MerchantRank rank = rankProfile.getParentRank();
			Integer formNum = rankProfile.getDiamondProductNumber();
			
			if (rankProfile.getId() != null){
				rankProfile = this.merchantRankProfileService.getById(rankProfile.getId());
				isNewProfil = false;
			}else{
				isNewProfil = true;
			}
			if (formNum != null){
				rankProfile.setDiamondProductNumber(formNum);
			}
			if (rank != null){
				rank = this.merchantRankService.getById(rank.getId());
				rankProfile.setParentRank(rank);
			}
			if (isNewProfil){
				merchantRankProfileService.create(rankProfile);
			}else{
				merchantRankProfileService.update(rankProfile);
			}
		}
		store.setRankProfil(rankProfile);
		//get country
