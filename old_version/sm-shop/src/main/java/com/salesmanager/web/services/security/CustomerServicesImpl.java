...
public class CustomerServicesImpl implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String userName)
		...
		try {
			...
			// change1: clariones for new auto-login both buyer&seller requirement
			...
			addSellerRoles(customer, groupsId);
	...
	private void addSellerRoles(Customer customer, List<Integer> groupsId) {
		String phone = customer.getPhone();
		if (phone == null){
			return;
		}
		try {
			com.salesmanager.core.business.user.model.User user = userService.getByUserMobile(phone);
			if (user == null){
				return;
			}
			...
			e.printStackTrace();
			LOGGER.warn("Cannot login seller with mobile number "+ phone);
			// cannot got seller when login buyer, just ignore exception
		}
