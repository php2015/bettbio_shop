...
import com.salesmanager.web.admin.controller.products.facade.OptionValueFacade;
...
public class OptionsValueController {
	...
	@Autowired
	OptionValueFacade optionValueFacade;
	...
	@RequestMapping(value="/admin/optionvalues/list.html", method={RequestMethod.GET,RequestMethod.POST})
	public String displayOptions(@Valid @ModelAttribute("criteria") Criteria criteria,Model model, HttpServletRequest request, HttpServletResponse response,@RequestParam(value = "page", defaultValue = "1") final int page) throws Exception {
		...
		ReadableOptionValueList olist = optionValueFacade.getByCriteria(criteria, language);
		
		model.addAttribute( "values", olist);
		if(olist!=null) {
		...
		setMenu(model,request,"catalogue-optionsvalues-list");
	...
	@PreAuthorize("hasRole('PRODUCTS')")
	@RequestMapping(value="/admin/optionvalues/removes.html", method=RequestMethod.POST, produces="application/json")
	public @ResponseBody String deleteOptionValues(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		try{
			...
			delteAll.setService(productOptionValueService);
	...
	private String displayOption(Long optionId, HttpServletRequest request, HttpServletResponse response,Model model,Locale locale) throws Exception {
		...
		if(optionId == null){
			this.setMenu(model, request,"catalogue-optionsvalues-create");
		}else{
			this.setMenu(model, request,"catalogue-optionsvalues-list");
		}
		MerchantStore store = (MerchantStore)request.getAttribute(Constants.ADMIN_STORE);
	...
	*/
	@PreAuthorize("hasRole('PRODUCTS')")
	@RequestMapping(value="/admin/optionvalues/remove.html", method=RequestMethod.POST, produces="application/json")
