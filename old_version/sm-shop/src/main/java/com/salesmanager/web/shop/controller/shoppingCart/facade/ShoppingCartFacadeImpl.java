    ...
    implements ShoppingCartFacade
{
    ...
    public void deleteShoppingCart(final Long id, final MerchantStore store, final Customer customer) throws Exception {
    ...
    @Override
    public void deleteShoppingCart(final String code, final MerchantStore store, final Customer customer) throws Exception {
        ...
        throws Exception
    {
        ...
        {
            // get it from the db
            cartModel = getShoppingCartModel( item.getCode(), store , customer);
            if ( cartModel == null )
            {
        ...
        shoppingCartCalculationService.calculate(customer, cartModel, store, language );

        ShoppingCartDataPopulator shoppingCartDataPopulator = new ShoppingCartDataPopulator();
        shoppingCartDataPopulator.setShoppingCartCalculationService( shoppingCartCalculationService );
        shoppingCartDataPopulator.setPricingService( pricingService );
        shoppingCartDataPopulator.setCustomer(customer);
        shoppingCartDataPopulator.setBrandDiscountService(brandDiscountService);
    ...
    @Override
    public ShoppingCartData getShoppingCartData( final ShoppingCart shoppingCartModel ,final Customer customer)
        throws Exception
    {
    ...
    @Override
    public ShoppingCartData removeCartItem( final Long itemID, final String cartId ,final MerchantStore store,final Language language ,final Customer customer)
        throws Exception
    {
    ...
    @Override
    public ShoppingCartData updateCartItem( final Long itemID, final String cartId, final long newQuantity,final MerchantStore store, final Language language ,final Customer customer)
        throws Exception
    {
        if ( newQuantity < 1 )
        {
        ...
        if ( StringUtils.isNotBlank( cartId ) )
        {
            ...
            if ( cartModel != null )
            {
                ...
                shoppingCartDataPopulator.setShoppingCartCalculationService( shoppingCartCalculationService );
                shoppingCartDataPopulator.setPricingService( pricingService );
                shoppingCartDataPopulator.setCustomer(null);
                shoppingCartDataPopulator.setBrandDiscountService(brandDiscountService);
                return shoppingCartDataPopulator.populate( cartModel, store, language );
    ...
    @Override
    public ShoppingCartData updateCartItems( final List<ShoppingCartItem> shoppingCartItems, final MerchantStore store, final Language language,final Customer customer )
            throws Exception
        {
	...
	@Override
	public ShoppingCartData getShoppingCartData(String code, MerchantStore store,final Customer customer) {
		try {
			ShoppingCart cartModel = shoppingCartService.getByCode( code, store );
			if(cartModel!=null) {
				ShoppingCartData cart = getShoppingCartData(cartModel, customer);
				return cart;
			}
		} catch( NoResultException nre) {
	        	//nothing
	...
	public ShoppingCart getShoppingCartModel(String shoppingCartCode,
			MerchantStore store,final Customer customer) throws Exception {
	...
	public ShoppingCartData updateModelCartItems(
		...
		if(cartItems == null){
			throw new CartModificationException( "ShoppingCart is empty" );
		}
		...
		for(com.salesmanager.core.business.shoppingcart.model.ShoppingCartItem item : shoppingCartItems) {
           ...
           // finalPrice.setProductPrice(item.getProductPrice());
            ...
            List<ProductPricingData> prodPricDatas = pricingService.calculateProductListPrice(priceCtx, Arrays.asList(new Long[]{entryToUpdate.getProductId()}));
            if (prodPricDatas != null){
            	ProductPricingData ppd = ProductPriceUtils.getProductPriceData(prodPricDatas, entryToUpdate.getProductId());
            	if (ppd != null){
            		ProductSpecPricingData spd = ProductPriceUtils.getProductSpecPrice(ppd, entryToUpdate.getPriceId(), entryToUpdate.getSpec());
            		if (spd != null && spd.getFinalPrice() != null){
            			entryToUpdate.setItemPrice(spd.getFinalPrice());
            		}
            	}
            }
            //计算单商品总价
            ...
            entityDateItem.setProductCode(entryToUpdate.getProduct().getCode());
	...
	public ShoppingCartData addItemsToShoppingCart(
        ...
        {
            // get it from the db
            cartModel = getShoppingCartModel(code, store , customer);
            if ( cartModel == null )
            {
