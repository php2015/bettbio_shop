...
//		MerchantStore store = (MerchantStore)request.getAttribute(Constants.MERCHANT_STORE);
	...
	@RequestMapping("/shop/activity.html")
	public String activity( Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		MerchantStore store = (MerchantStore)request.getAttribute(Constants.MERCHANT_STORE);
		StringBuilder template = new StringBuilder().append("activity").append(".").append(store.getStoreTemplate());
		return template.toString();
	}
	
	@RequestMapping("/shop/brands.html")
	public String brands( Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		MerchantStore store = (MerchantStore)request.getAttribute(Constants.MERCHANT_STORE);
		model.addAttribute("brandBanners", brandService.getFullBrandBannerList());
		StringBuilder template = new StringBuilder().append("brands").append(".").append(store.getStoreTemplate());
		return template.toString();
	}
}
