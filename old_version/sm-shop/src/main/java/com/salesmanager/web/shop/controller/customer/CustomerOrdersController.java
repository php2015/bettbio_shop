...
public class CustomerOrdersController extends AbstractController {
	...
	public @ResponseBody String savePayProofImage(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if (orderId != null) {
				Order order = orderServiceV2.getById(Long.valueOf(orderId));
				OrderServiceV2Context orderCtx = OrderUtils.getOrderContext(request, customerFacade, null);
				orderCtx.setOrder(order);
				orderCtx.setUser(null);
				String oldImage = orderServiceV2.savePayProof(orderCtx, Long.parseLong(suborderId), imageFileName, false);
			...
			e.printStackTrace();
			String msgStr = messages.getServiceExceptionV2Message(e, OrderServiceV2Constants.ERROR_MSG_PREFIX, "上传支付凭证失败", locale);
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
			resp.setStatusMessage(msgStr);
		} catch (Exception e) {
			e.printStackTrace();
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
