...
public class ManufacturerController {
	...
	public String saveManufacturer(
			@Valid @ModelAttribute("manufacturer") com.salesmanager.web.admin.entity.catalog.Manufacturer manufacturer,
			BindingResult result, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale)
					throws Exception {

		this.setMenu(model, request,"manufacturer-create");
		//save or edit a manufacturer
		MultipartHttpServletRequest mureq = (MultipartHttpServletRequest) request;
		MultipartFile file = mureq.getFile("imageFile");
		...
		if (file == null || file.getSize() == 0){
			// 提交的数据里没有文件 那么需要删除原来的文件
			String oldImageFileName = newManufacturer.getImage();
			if (oldImageFileName != null && (fileName==null || fileName.isEmpty())){
				// 如果以前有文件，现在文件名也不一样，那还的先把之前文件删除
				imageStorage.removeImage(oldImageFileName);
				newManufacturer.setImage(null);
			}
		}else{
			// 有上传文件
			...
			if (contentType != null){
				...
				postfix = "." + contentType.substring(pos+1);
			}
			String imageFileName = manufacturerService.getLogoImageUri(newManufacturer, postfix);
			//看以前有没有文件
			String oldImageFileName = newManufacturer.getImage();
			if (oldImageFileName != null && !oldImageFileName.equals(imageFileName)){
				// 如果以前有文件，现在文件名也不一样，那还的先把之前文件删除
				imageStorage.removeImage(oldImageFileName);
			}
			...
			newManufacturer.setImage(imageFileName);
		}
