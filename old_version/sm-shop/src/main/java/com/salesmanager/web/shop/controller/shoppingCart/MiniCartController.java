...
public class MiniCartController extends AbstractController{
	...
	public @ResponseBody ShoppingCartData removeShoppingCartItem(Long lineItemId, final String shoppingCartCode, HttpServletRequest request, Model model) throws Exception {
		...
		ShoppingCartData shoppingCartData=shoppingCartFacade.removeCartItem(lineItemId, cart.getCode(), merchantStore,language, customer);
		
		
		if(CollectionUtils.isEmpty(shoppingCartData.getSubOrders())) {
			shoppingCartFacade.deleteShoppingCart(shoppingCartData.getId(), merchantStore, customer);
