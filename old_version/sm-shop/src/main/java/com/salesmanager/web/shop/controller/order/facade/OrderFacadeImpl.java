 ...
 */
			...
			for(SubOrder sub:subOrders){
				modelOrder.setStoreName(sub.getStoreName());
	...
	public ReadableOrderList getReadableOrderList(OrderCriteria criteria,Language language) throws Exception {
    	OrderList orderList = orderServiceV2.listByCriteria(criteria);
    ...
    private EzybioOrder popEzybio(Order order,Language language,boolean isALL){
    	...
    	if (customer != null){
    		ezybioOrder.setBuyerCompany(customer.getCompnay());
    	}
    ...
    private com.salesmanager.web.entity.order.SubOrder addSubOrder(SubOrder subOrder,Language language){
		...
		returnSubOrder.setStoreName(subOrder.getStoreName());
		returnSubOrder.setPayProof(subOrder.getPayProof());
    		...
    		for(OrderProduct product:products){
    			...
    			if(product1 != null){
                    ...
                    if(image!=null) {
                    	...
                    	Category cat = product1.getCategories().iterator().next();
                    	if(cat !=null){
                    		String cCode = cat.getCode();
                    		int ctype=1;
                    		if(cCode.startsWith("04")){
                    			ctype= 4;
                    		}else if(cCode.startsWith("02")){
                    			ctype= 2;
                    		}else if(cCode.startsWith("03")){
                    			ctype= 3;
                    		}                    		
	...
	public EzybioOrder getEzybioOrder(Long orderId,Language language,long storeID) throws Exception {
		...
		if(storeID==-1l){
			modelOrder = orderServiceV2.getById(orderId);
		}else{
			modelOrder = orderServiceV2.getByIdByStore(orderId, storeID);
			isAll = false;
		}
		if(modelOrder==null) {
		...
		ezybioOrder.setStoreName(modelOrder.getStoreName());
		return ezybioOrder;
	}
	/**
	...
	public ReadableOrderList getStoreReadableOrderList(OrderCriteria criteria,
		...
		// TODO Auto-generated method stub
		OrderList orderList = orderServiceV2.listStoreByOrderCriteria(criteria);
	...
	private void setOrderDelivery(long orderid){
		...
		List<OrderStatus> subStatus = new ArrayList<OrderStatus>(subs.size());
		for(SubOrder sub : subs){
			subStatus.add(sub.getStatus());
		}
		if (order.getStatus().equals(OrderStatus.calcFinalStatus(subStatus))){
			return;
		}
