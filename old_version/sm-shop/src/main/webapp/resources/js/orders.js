function findByStatus(status1, status2){
	$("#status").val(status1);
	$("#secondStatus").val(status2);
	dosubmit();
};
function findByYear(yearVal){	
	$("#beginDatePurchased").val(yearVal) 
	dosubmit();
...
/**
 * confirm  the money has paid
 */
function confirmPaid(oid, soid){
	$('#confirmPaid').modal('show');
	$('#closeOrder').bind('click', function(){
		$('#confirmPaid').modal('hide');
		$.ajax({  
			type: 'POST',
			url: getContextPath()+"/admin/orders/confirmPaid.html",
			data:{
				orderid:oid,
				suborderid:soid
			},
