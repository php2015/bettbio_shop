...
function createTable(ps){
	...
	$.each(ps, function(i, p) {
        ...
        tbBody +=  p.productName + '</a>';
		if (p.diamondProduct){
			tbBody += '<span class="diamond_product_flag">*</span>'
		}
		tbBody += '</td><td style=" word-break: break-all;">'+p.productEnName+'</td><td style=" word-break: break-all;">' + p.sku + '</td><td style=" word-break: break-all;">' + p.code + '</td><td>'+getAudit(p.audit)+'</td><td>'+quality+'</td><td><span id=av_'+p.id+'>' + p.available + '</span><div class="pull-right"><input type="checkbox" onclick="changeav(this)" id='+p.id+' '+checked+'></div></td><td><span>'+p.availableDate+'</span></td><td>'+ p.storeName  
        tbBody +=  '</td><td>'+'<a href="javascript:void(0);" onclick="delproduct('+p.id+')">'+deltile+'</a>';
