...
function Dropify(element, options) {
    ...
    var me = this;
    element.getDropify = function () { return me;};
    this.input           = $(this.element);
    this.wrapper         = null;
    this.preview         = null;
    this.filenameWrapper = null;
    this.settings        = $.extend(true, defaults, options, this.input.data());
    this.imgFileFormats  = ['png', 'jpg', 'jpeg', 'gif', 'bmp','pdf'];
    this.file            = null;
    this.filename        = null;
...
/*Dropify.prototype.removeImage = function(id){
	$.ajax({
		url:'/sm-shop/admin/products/deleteProductImage.html',
		type:'post',
		data:{
			id:id
		},
		...
		success:function(data){
			if(data.response.status==0 || data.response.status=="0")
			{
...
Dropify.prototype.setPreview = function(src)
{
    ...
    if (this.isImage() === true) {
        $('<img />').attr('src', src).appendTo(render).css({
    	 ...
    	 if(this.getFilename(src)==''){
    		 return true;
    	 }else{
    		 alert("对不起，您添加的图片格式不正确，请输入['png', 'jpg', 'jpeg', 'gif', 'bmp','pdf']格式");
    	    return false; 
    	 }
    }
