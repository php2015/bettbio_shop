							...
							<c:forEach items="${currentMenu.menus}" var="menu" varStatus="z">
	  							...
	  							<c:choose>
	  							    <c:when test="${fn:length(menu.menus)==0}">
	  									<li id="${menu.code}-tab" <c:if test="${activeMenus[menu.code]!=null}"> class="active"</c:if>>
											<a href="#" id="${menu.code}-link" data-toggle="tab"><s:message code="menu.${menu.code}" text="${menu.code}"/>
											</a>
											<c:if test="${menu.code == 'diamond-product'}">
												<img src="<c:url value="/resources/img/NEW.gif"/>" style="position: absolute;left: 60px;top: 0px;"/>
											</c:if>								
										</li>
