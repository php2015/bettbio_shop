			...
			<table id ="storelist" class="table table table-striped table-bordered table-hover">
				...
				<thead>
					<tr >
						...
						<th><s:message code="label.store.seed" text="Seed"/><div class="pull-right"><div class="pull-right"><input type="checkbox" id="allAv"></th>
					...
					<c:forEach items="${stores.stores}" var="store" varStatus="storeStatus">
						...
						<tr id="result_name_${store.id}" class="everyTR">
							...
							<td ><a href="<c:url value="/admin/store/editStore.html?id=" />${store.id}">${store.code}</a></td>
								...
								<td ><span id="av_${store.id}">${store.seeded}</span><div class="pull-right"><input type="checkbox" onclick="changeav(this)" id="${store.id}" <c:if test="${store.seeded == true}"> checked="checked"</c:if>></div></td>
