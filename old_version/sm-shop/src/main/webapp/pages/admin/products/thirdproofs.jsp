...
var pagingUrl = '<c:url value="/admin/products/thirdproofs/paging.html?productId=${product.id}"/>';
var editUrl = '<c:url value="/admin/products/thirdproof/edit.html?productId=${product.id}"/>';
var removesUrl = '<c:url value="/admin/products/thirdproof/removes.html?productId=${product.id}"/>';
$(function(){
	initCheckbox();
	loadDataList();
});
function loadDataList() {
	...
	$.post(pagingUrl, {}, function(result){
		if(result.response.status == 0) {//RESPONSE_STATUS_SUCCESS
			...
			$("#thirdproofsTbody").empty();
			$(list).each(function(index){
				var tbBody="";
				tbBody += '<tr id="result_name_'+this.thirdproofId+'" class="everyTR">';
				tbBody += '<td><input type="checkbox" class="everycheckbox" id="'+this.thirdproofId+'"></td>';
				tbBody += '<td><a href="'+editUrl+'&id='+this.thirdproofId+'">'+this.thirddetection+'</a></td>';
				tbBody += '<td>'+this.order+'</td>';
		        tbBody += '<td>'+'<a href="javascript:void(0);" onclick="removefun('+this.thirdproofId+')"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a></td>';
		        tbBody += '</tr>';
				$("#thirdproofsTbody").append(tbBody);
			});
		} else {
...
function removefun(thirdproofId) {
	
	$("#confirmHref").unbind("click").bind("click",function(){
		$.post(removesUrl, {thirdproofId: thirdproofId}, function(result){
...
<div class ="row">
	...
	<div id="p-title">
		<a class="btn btn-primary" href="<c:url value="/admin/products/thirdproof/create.html?productId=${product.id}&availabilityId=${availability.id}"/>"><s:message code="label.product.thirdproof.create" text="Create thirdproof" /></a>	
		<br/>
		<!-- begin search box -->
		...
		<table class="table table-striped table-bordered table-hover "
			id="CustmoersTable">
			<thead id="thirdproofsTable">
				<tr class="cubox">
					<th><input type="checkbox" id="selectall"></th>
					<th><s:message code="label.product.thirdproof.thirddetection" text="Thirddetection"/></th>
					<th><s:message code="label.entity.order" text="Order"/></th>
					<th><s:message code="label.generic.remove" text="Del" /></th>
				</tr>
			</thead>
			<tbody id="thirdproofsTbody">
			</tbody>
		</table>
		<!-- end content list -->
