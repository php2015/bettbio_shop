...
<div class="tabbable">
		 ...
		 <jsp:include page="/common/adminTabs.jsp" />
				...
				<form:form method="POST" commandName="manufacturer" action="${manufacturerSave}" class="form-horizontal" enctype="multipart/form-data">
					...
					<c:forEach items="${manufacturer.descriptions}" var="description" varStatus="counter">
						...
						<div class="form-group form-group-lg">
							<label class="required col-sm-2 control-label"><s:message code="label.manufacturer.image" text="Manufacturer LOGO Image"/></label>
							<div class="col-sm-10">
								 <input type="file" name="imageFile" id="input_logo_image"  class="dropify-event" 
									<c:if test="${not empty manufacturer.manufacturer.image}">
										data-default-file="/sm-shop/<c:url value='${manufacturer.manufacturer.image}'/>" 
									</c:if>
									></input>
								<input type="hidden" name="imageFileName" id="input_logo_image_name" value="<c:url value='${manufacturer.manufacturer.image}'/>"/>
							</div>
						</div>
