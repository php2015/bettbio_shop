...
public class ShippingServiceImpl implements ShippingService {
	...
	public ShippingQuote getShippingQuote(MerchantStore store, Customer customer, List<ShippingProduct> products, Language language) throws ServiceException  {
		...
		try {
			...
			//delivery change to custmeraddress
			
			//look if customer country code excluded
			//modify by cy for muti delivery
			/**
			...
			*/
			...
			// modify by cy now no international 
			//free shipping ?
			...
			try {
				shippingOptions = shippingQuoteModule.getShippingQuotes(packages, orderTotal,  store, configuration, shippingModule, shippingConfiguration, locale);
			} catch(Exception e) {
			...
			if(shippingOptions!=null) {
				...
				for(ShippingOption option : shippingOptions) {
					...
					if(StringUtils.isBlank(option.getOptionName())) {
						// modify by cy for use muti deliery now only china
						...
						if(countryName == null) {
							...
							option.setOptionName(China.getName());		
					}
