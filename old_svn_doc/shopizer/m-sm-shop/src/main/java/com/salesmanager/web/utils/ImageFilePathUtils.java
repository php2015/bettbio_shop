...
public class ImageFilePathUtils {
	...
	/**
	 * Builds a product certificate or proof related image file path that can be used by image servlet
	 * @param store
	 * @param product
	 * @param imageName
	 * @param prefix
	 * @return
	 */
	public static String buildProductRelatedImageFilePath(MerchantStore store, Product product, String imageName, String prefix) {
		return new StringBuilder().append(Constants.STATIC_URI).append("/").append(store.getCode()).append("/").append(FileContentType.PRODUCT.name()).append("/")
				.append(product.getSku()).append("/").append(prefix).append("/").append(imageName).toString();
	}
