	...
	$.fn.fsvs = function( options ) {
		...
		var hasTransition = function(){
		    prefixes = ['Webkit','Moz','ms','O'];
		   	for( var i in prefixes ) {
		   		if( typeof document.getElementsByTagName( 'ezybiomainbody' )[0].style[prefixes[i] + 'Transition' ] !== 'undefined' ) {
		   			return true;
		   		}
		   	}
		    return false;
		...
		var removeStyling = function(){
			...
			$('> div', body).each(function(i){
				$(this).attr('class', 'bettslide' );
			});
			$('ezybiomainbody').attr('class','');
		...
		var app = {
			...
			addPagination : function() {
				...
				pagination.appendTo( $('ezybiomainbody') );
			...
			shouldRun : function() {
				return $('ezybiomain').hasClass( 'fsvs' );
			},
