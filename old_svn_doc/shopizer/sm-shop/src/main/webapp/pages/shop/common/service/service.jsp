...
<script type="text/javascript">
  $(function(){
      $(window).resize(function(){
         $("#img_title").css("marginLeft",""+(parseInt($(window).width())-90)+"px");
         $("#img_title a").children("img").css({"marginLeft":"-6px"});
      });
      $(window).load(function(){
         $("#img_title ul li a img").css({"width":"20px","height":"20px","marginLeft":"-3px"});
      });
  });
</script>
<script type="text/javascript">
...
<div style="width:1200px;border: 0px solid #804040;margin-left: auto;margin-right: auto;padding:20px 0px;">
    <div class="row" style="margin-top: -30px; width: 100%">
		<ul class="breadcrumb" style="background-color:transparent !important; width: 1200px; box-shadow: none;padding-top:30px;">
			...
			<div id="iamges" style="width:352px;height: 352px;">
				<c:choose>
					...
					<a href="<c:url value="/shop/dispalyImage.html?ipath=${product.image.imageUrl}"/>" target="blank">
					   <img style="padding-top:35px;max-width:350;max-height:350px;" alt="<c:out value="${product.description.name}"/>" src="<c:url value="${product.image.imageUrl}"/>"  id="imagUrl">
					</a>
					</c:when>
					<c:otherwise>
						<img src="<c:url value="/resources/img/04.png" />"  style="max-width: 340px;max-height: 340px"/>
					</c:otherwise>
				</c:choose>	
				</div>
				<br/>
			 <div>
			   ...
			   <div id="imgUrl">
			    ...
			    <c:if test="${product.imagelength==0}">
			      <a href="javascript:void();" title="/resources/img/04.png""> <img id="" src="<c:url value="/resources/img/04.png"/>" style="max-width:60px;max-height:60px;margin-left: 2px" /></a>
			    </c:if>
			    </div>
			 </div>
			</td>
			<td style="width:550px;padding-left:30px;;" valign="top">
				...
				<%-- <c:choose>
					<c:when test="${not empty  product.manufacturer && not empty product.manufacturer.manName}">
						<div style="padding-top:22px;font-size:14px;">
							品牌：<span> ${product.manufacturer.manName}</span>
							<c:if test="${not empty  product.batchnum}">
								产地：<span class="pull-right">${product.batchnum}</span>
							</c:if>
						</div>
					</c:when>
					<c:otherwise>
					...
					<c:when test="${not empty  product.code}">
						<div style="padding-top:22px;font-size:14px;">
							...
							<%-- <c:if test="${not empty  product.cas}">
								型号：<span class="pull-right">${product.cas}</span>
							</c:if> --%>
						</div>
					</c:when>
					<c:otherwise>
				...
				<div style="padding-top:22px;font-size:14px;" class="pull-right">
					...
					<span><button style="height:40px;width:120px;display:none" type="button" id="bntRel" class="btn btn-large btn-danger " >联系商家</button></span>
			    	...
			    	<li role="presentation" class="active">
		               ...
		               <%-- <c:if test="${reviews!=null}">
		                ...
		                <c:if test="${not empty product.thirdProofs }">
		              		 <li role="presentation" ><a href="#section-4" role="tab" data-toggle="tab"> <s:message code="label.product.thirdproofs" text="Third"/></a></li>
		               </c:if>
		                <c:if test="${not empty product.productProofs }">
		               		<li role="presentation" ><a href="#section-5" role="tab" data-toggle="tab"><s:message code="label.product.proofs" text="Proof"/></a></li>
		               </c:if>
				...
				<div  class="tab-content" id="tab-content">
					 <div role="tabpanel" class="tab-pane active detailWidth"  id="section-1">
			            	<div style="padding:20px;">
								<%-- <span class="label " style="padding:10px 30px;background-color:#ff313b;border-radius:10px;font-size:16px;z-index:0">
									<s:message code="label.productedit.productdesc" text="Desc"/>
								</span> --%>
							</div>
		            	...
		            	<div role="tabpanel" class="tab-pane detailWidth" id="section-2">
		            		<div style="padding:20px;">
								<span class="label " style="padding:10px 30px;background-color:#ff313b;border-radius:10px;font-size:16px;">
									<s:message code="label.product.customer.reviews" text="Thirdproof"/>
								</span>
							</div>
				         <div class="container">
				      ...
				      <c:if test="${not empty product.productCertificates }">
			            <div role="tabpanel" class="tab-pane detailWidth" id="section-3">
			            	<div style="padding:20px;">
								<%-- <span class="label " style="padding:10px 30px;background-color:#ff313b;border-radius:10px;font-size:16px;">
									<s:message code="label.product.certificate" text="Certifate"/>
								</span> --%>
							</div>	            	
				           <div class="container">
								<div class=" ">
				          ...
				          <c:if test="${not empty product.thirdProofs }">
				            <div role="tabpanel" class="tab-pane detailWidth" id="section-4">
				            	<div style="padding:20px;">
									<%-- <span class="label " style="padding:10px 30px;background-color:#ff313b;border-radius:10px;font-size:16px;">
										<s:message code="label.product.thirdproofs" text="Third"/>
									</span> --%>
								</div>
				          	<div class="container">
									<div class=" ">
			             ...
			             <c:if test="${not empty product.productProofs }">
			            	<div role="tabpanel" class="tab-pane detailWidth" id="section-5">
			            		<div style="padding:20px;">
									<%-- <span class="label " style="padding:10px 30px;background-color:#ff313b;border-radius:10px;font-size:16px;">
										<s:message code="label.product.proofs" text="Proof"/>
									</span> --%>
								</div>	            			
			            		<div class="container">
									<div class=" ">
...
<input type="hidden" value="${product.store.useQQ}" id="qquser"/>	
<input type="hidden" value="${product.store.qqNum}" id="qqNum"/>
<input type="hidden" value="http://wpa.qq.com/msgrd?v=3&uin=${product.store.qqNum}&site=qq&menu=yes" id="qqUrl"/>
</div>
<br>
<!-- end body -->            
<script>
jQuery(function($) {
  $(document).ready( function() {
       $("#bntRel").click(function(){
          if($("#qquser").val()==true){
             if($("#qqNum").val()!=null){
                window.location.href=""+$("#qqUrl").val()+"";
             }
          }
       });
	   getServerPrice(${product.id});
	   getProductFile(${product.id});
	//set cookie
  	var imageurl='/resources/img/04.png';
