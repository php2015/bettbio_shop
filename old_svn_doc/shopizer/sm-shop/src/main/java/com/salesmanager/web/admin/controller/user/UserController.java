...
public class UserController {
	...
	private void populateUserObjects(User user, MerchantStore store, Model model, Locale locale,boolean isSuperAdmin) throws Exception {
		...
		for(Group group : userGroups) {
			...
			if(group.getGroupName().equalsIgnoreCase(Constants.GROUP_ADMIN) ){
				if(isSuperAdmin)	groups.add(group);
			}else if(!group.getGroupName().equals(Constants.GROUP_SUPERADMIN)) {
				groups.add(group);
			}
		}
	...
	private String displayUser(User user, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		if(auth != null && request.isUserInRole("SUPERADMIN")){
			isSuperAdmin =true;
		}		
		if(user==null) {
			user = new User();
		...
		this.populateUserObjects(user, store, model, locale,isSuperAdmin);
	...
	public @ResponseBody String resetPassword(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		MerchantStore store = (MerchantStore)request.getAttribute(Constants.ADMIN_STORE);
		int result =emailTemplatesUtils.sendUserSetPasswordMail(uId,store,request.getContextPath());
