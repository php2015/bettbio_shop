...
public class ShoppingCategoryController {
	...
	public	List<ReadableProduct> getPrice(HttpServletRequest request, HttpServletResponse response) {
		...
		List<ReadableProduct> rplikst =null;
		...
		if (StringUtils.isNotBlank(productIds)) {
			...
			List<ProductAvailability> prices = productService.getPrice(criteria);
			
			if(prices != null){
				ReadableProductPopulator populator = new ReadableProductPopulator();
				populator.setPricingService(pricingService);
				MerchantStore store = (MerchantStore)request.getSession().getAttribute(Constants.MERCHANT_STORE);
				rplikst = populator.getPrice(prices, store);
			}			
		}
		//获取服务类价格
		if (!StringUtils.isBlank(services)){
			List<Object[]> ids = productService.getServicePrice(services);
			if(ids !=null && ids.size()>0){
				if(rplikst == null ) rplikst = new ArrayList<ReadableProduct>();
				for(Object id[] : ids){					
					if(id[1] != null ){
						ReadableProduct rp = new ReadableProduct();
						rp.setsPrice(id[1].toString());
						rp.setId(Long.parseLong(id[0].toString()));
						rplikst.add(rp);
					}
				}
			}
